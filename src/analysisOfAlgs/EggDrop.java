...
public class EggDrop {
	 ...
	 *              the first time - that means T is in the interval between 2^n
	 *              and 2^(n-1)
	 */
	public int version2() {
		Egg egg = new Egg();
		int lo = 0, hi = N - 1;
		int limit = (int) Math.pow(2, (int) ((Math.log10(N)/Math.log10(2)) + 1));
		...
		if (egg.isBroken) {
			...
			int eggNum = (int) Math.ceil((Math.log10(interval)/(Math.log10(2))));
			Egg[] eggs = new Egg[eggNum];
			System.out
					.println("Version 2 number of eggs used to determine interval of size < T: 1, number of eggs for binary search: "
							+ eggNum);
			for (int i = 0; i < eggNum; i++) {
				Egg eggNew = new Egg();
				eggs[i] = eggNew;
